...
import { Calendar, TwitterIcon, UserPlus, Users } from "lucide-react";
...
const extractInfoFromText = (text: string): ExtractedInfo => {
  ...
  const regexPatterns: Record<
    keyof Omit<ExtractedInfo, "bio" | "username">,
    RegExp
  > = {
    name: /\| name:\s*([^|]+)/,
...
export default function ProfileDisplay({
  ...
  return (
    <div className="w-full bg-white dark:bg-gray-900 border dark:border-gray-800 shadow-sm rounded-none overflow-hidden">
      {/* Header Banner */}
      ...
      <div className="px-6">
        ...
        <div className="pt-4">
          ...
          {username && (
            <h2 className="text-gray-500 dark:text-gray-400">@{username}</h2>
          )}
        </div>
        ...
        {username && (
          <div className="flex justify-start mt-4 mb-4">
            <a
              ...
              className="px-4 py-1.5 rounded-full border border-gray-300 dark:border-gray-700 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors dark:text-gray-200"
            >
              View on X / Twitter
            </a>
          </div>
