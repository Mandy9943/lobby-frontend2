...
export default function CompetitorsDisplay({
  competitors,
  ...
  return (
    <div>
      <h2 className="text-2xl font-normal pb-4 text-slate-200">
        Similar Companies
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-6">
        {visibleCompetitors.map((competitor) => (
          <div
            key={competitor.url}
            className="bg-slate-900/50 backdrop-blur-sm p-6 border border-slate-800/50 rounded-lg hover:ring-brand-default hover:ring-1 transition-all duration-200"
          >
            <a
              href={competitor.url}
              target="_blank"
      ...
      {hasMore && (
        <div className="flex justify-start mt-6">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm text-slate-400 hover:text-slate-200 transition-colors duration-200"
          >
