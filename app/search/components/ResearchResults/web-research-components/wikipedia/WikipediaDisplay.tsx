...
const extractCompanyName = (url: string): string => {
  try {
    ...
    toast.error("Error extracting company name");
    return "";
  }
};
...
const isCompanyWikipedia = (wikiUrl: string, companyName: string): boolean => {
  try {
    ...
    toast.error("Error checking if company name exists in Wikipedia URL");
    return false;
  }
};

const WikipediaDisplay: React.FC<WikipediaDisplayProps> = ({
  data,
  websiteUrl,
}) => {
  ...
  return (
    <div className="bg-slate-900/50 backdrop-blur-sm rounded-lg shadow-lg border border-slate-800/50 p-8 transition-all duration-300 hover:shadow-xl hover:border-slate-700/50">
      {/* Header */}
      <div className="flex items-center gap-4 mb-8 border-b border-slate-800/50 pb-4">
        <div className="bg-slate-800 p-3 rounded-full">
          <FaWikipediaW className="text-2xl text-slate-300" />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-slate-200">Wikipedia</h2>
      ...
      {Object.keys(companyInfo).length > 0 && (
        <div className="mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-lg">
            {Object.entries(companyInfo).map(
              ([key, value]) =>
                value && (
          ...
          {timeline.length > 3 && (
            ...
            >
              {showAllEvents ? (
                ...
                <>
                  Show More ({timeline.length - 3} more events){" "}
      ...
      {data.url && (
        <a
          ...
          className="inline-flex items-center gap-2 bg-slate-800 text-slate-300 px-4 py-2 rounded-full hover:bg-slate-700 transition-colors mt-4"
        >
          <FaWikipediaW />
