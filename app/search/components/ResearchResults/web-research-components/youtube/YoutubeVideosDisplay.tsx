...
export default function YoutubeVideosDisplay({
  videos,
}: YoutubeVideosDisplayProps) {
  ...
  useEffect(() => {
    const fetchVideoDetails = async () => {
      const videoIds = validVideos
        .map((video) => getVideoId(video.url))
        .filter(Boolean);
      try {
        const response = await fetch(
          `${researchSearchApiUrl}/youtube-details?ids=${videoIds.join(",")}`
        );
        if (response.ok) {
          ...
          {validVideos.map((video, index) => {
            ...
            return (
              <motion.a
                ...
                className="flex-shrink-0 w-[350px] group bg-slate-900/50 backdrop-blur-sm rounded-lg border border-slate-800/50 shadow-lg hover:shadow-xl hover:border-slate-700/50 transition-all duration-300 overflow-hidden"
                  ...
                  {details && (
                    <>
                      ...
                      <p className="text-xs text-slate-500">
                        ...
                        {getRelativeTime(details.publishedAt)}
                      </p>
                    </>
                  )}
                </div>
