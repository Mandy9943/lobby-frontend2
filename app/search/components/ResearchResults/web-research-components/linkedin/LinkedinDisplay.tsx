...
function processLinkedInText(data: LinkedInData): ProcessedData {
  ...
  const description =
    aboutIndex !== -1 ? extract("About us").split("\n")[0].trim() : "";
...
export default function LinkedInDisplay({ data }: { data: LinkedInData }) {
  ...
  return (
    <div className="bg-slate-900/50 backdrop-blur-sm p-6 md:p-8 w-full mx-auto border border-slate-800/50 shadow-lg hover:shadow-xl hover:border-slate-700/50 transition-all duration-300">
      <div className="flex flex-col md:flex-row items-start gap-6 mb-8">
        {processedData.logo && (
          <div className="w-20 h-20 md:w-24 md:h-24 relative flex-shrink-0 mx-0 bg-white rounded-lg p-2">
            <Image
